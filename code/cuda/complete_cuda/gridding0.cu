    ...
    for (uint32_t idual = 6 * blockIdx.x; idual < (ng - 3);
      ...
      for (uint32_t ifine = 0; ifine < 3; ifine++) { // fine-grid
        ...
        if (i > ng - 4)
          break;
        fprev=0;
        for (int x = 0; x < 4 * 4 * 3; x++) {
          partial_sum[x * BlockSizeD2 + tid] = 0;
        }
        // loop through all points inside box
        ...
        for (uint32_t TID = threadIdx.x; TID < ib[i + 1] - ib[i];
          ...
          f2 = (uint32_t)floor(y[ib[i] + TID + nPts]);
          d = y[ib[i] + TID + nPts] - (coord)f2;
          ...
          if(f1!=fprev){
            //__syncthreads();
            Reduce2V2D(idx1, idx2,partial_sum, V,f1,fprev, tid, nVec, ng);
            for (int x = 0; x < 4 * 4 * 3; x++) {
              partial_sum[x * BlockSizeD2 + tid] = 0;
            }
            fprev=f1;
          ...
          for (uint32_t j = 0; j < nVec; j++) {
            for (idx2 = 0; idx2 < 4; idx2++) {
              ...
              coord qv = q[nPts * j + ib[i] + TID] * v2[idx2];
