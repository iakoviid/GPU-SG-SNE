...
void bsr1run(float *val, int *block_ptr, int *col_ind, float *y, float *Fattr,
  ...
  float *cu_y;
  float *cu_F;
  CudaCheck(cudaMalloc(&cu_y, rows * d * sizeof(float)));
  CudaCheck(
      cudaMemcpy(cu_y, y, rows * d * sizeof(float), cudaMemcpyHostToDevice));
  CudaCheck(cudaMalloc(&cu_F, rows * d * sizeof(float)));
  CudaCheck(cudaMemcpy(cu_F, Fattr, rows * d * sizeof(float),
                       cudaMemcpyHostToDevice));
...
void tsne_cuda_spmv(float* Fattr,float* coov,float* Y,int* cooi,int* cooj, int n,int nnz,int d){
  ...
  int* cooi_d,*cooj_d;
  CudaCheck(cudaMalloc(&Fattr_d, n * d * sizeof(float)));
  CudaCheck(cudaMemcpy(Fattr_d, Fattr, n * d * sizeof(float), cudaMemcpyHostToDevice));
  CudaCheck(cudaMalloc(&Y_d, n * d * sizeof(float)));
  CudaCheck(cudaMemcpy(Y_d, Y, n * d * sizeof(float),cudaMemcpyHostToDevice));

  CudaCheck(cudaMalloc(&cooi_d, nnz * sizeof(int)));
  CudaCheck(cudaMemcpy(cooi_d, cooi, nnz * sizeof(int), cudaMemcpyHostToDevice));
  CudaCheck(cudaMalloc(&cooj_d, nnz * sizeof(int)));
  CudaCheck(cudaMemcpy(cooj_d, cooj, nnz * sizeof(int), cudaMemcpyHostToDevice));
  CudaCheck(cudaMalloc(&coov_d, nnz * sizeof(float)));
  CudaCheck(cudaMemcpy(coov_d, coov, nnz * sizeof(float), cudaMemcpyHostToDevice));
  ...
  CudaCheck(cudaMemcpy(Fattr, Fattr_d, n *d * sizeof(float), cudaMemcpyDeviceToHost));
