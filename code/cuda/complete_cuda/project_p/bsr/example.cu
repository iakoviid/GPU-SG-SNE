...
double compute_BSRkernel(BCRSArrays &bcsr, double *x, double *y) {
  ...
  grid_size.x = (bcsr.m * 32 + block_size.x - 1) / block_size.x;
...
void computeWithAll(COOArrays &coo, double times[4], double errors[4],
                    const int loop = 1, const int blockSize = 7) {
  CRSArrays crs;
  COO_to_CRS(coo, &crs);
  printf("Conversion Done 1\n");

  BCRSArrays bcsr;
  CRS_to_BCRS(crs, &bcsr, blockSize);
  printf("Conversion Done 2\n");
