...
__device__ uint32_t untangleLastDimDevice(int nDim, int TID, uint32_t qLevel,
                                          uint64_t *C) {
  uint64_t mask;
  switch (nDim) {
  case 1:
...
__device__ uint32_t untangleBoxIdx(int nDim, int TID, uint32_t qLevel,
                                   uint64_t *C,int nGrid) {
switch (nDim) {
  case 1:
    return untangleLastDimDevice(1, TID, qLevel, C);
  case 2:
    return untangleLastDimDevice(2, TID, qLevel, C)*nGrid+untangleLastDimDevice(1, TID, qLevel, C);
  case 3:
    return untangleLastDimDevice(3, TID, qLevel, C)*nGrid*nGrid+untangleLastDimDevice(2, TID, qLevel, C)*nGrid+untangleLastDimDevice(1, TID, qLevel, C);
}
...
__global__ void gridIdxKernel(uint32_t *ib, uint32_t *cb, uint64_t *C, int nPts,
  ...
  for (register int TID = threadIdx.x + blockIdx.x * blockDim.x; TID < nPts;
    ...
    if (TID < nPts - 1) {
      idxNew = untangleBoxIdx(nDim, TID, qLevel, C,nGrid);
      idxCur = untangleBoxIdx(nDim, TID + 1, qLevel, C,nGrid);
      if (idxNew != idxCur) {
        ib[idxCur] = TID + 1;
      }
      if (idxCur - idxNew > 1) {
...
__global__ void gridSizeAndIdxKernelNew(uint32_t *ib, uint32_t *cb, uint64_t *C,
  ...
  for (register int TID = threadIdx.x + blockIdx.x * blockDim.x; TID < nPts;
    ...
    idxNew = untangleLastDimDevice(2, TID, qLevel, C) * nGrid;
    idxNew += untangleLastDimDevice(1, TID, qLevel, C);

    if (TID > 0) {
      idxCur = untangleLastDimDevice(2, TID - 1, qLevel, C) * nGrid;
      idxCur += untangleLastDimDevice(1, TID - 1, qLevel, C);
    }
    if (idxNew != idxCur)
      ib[idxNew + 1] = TID + 1;
    // atomicAdd(&cb[idxNew], 1);
...
void relocateCoarseGrid(
  ...
  printf("nGrid=%d\n", nGrid);
  gridIdxKernel<<<32, 256>>>(ib, cb, Codes, nPts, nDim, nGrid, qLevel);
  cudaDeviceSynchronize();
