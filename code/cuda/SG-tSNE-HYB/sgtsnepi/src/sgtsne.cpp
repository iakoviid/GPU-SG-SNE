               ...
               double **timeInfo)
{
  ...
  double elapsedTime = (t2.tv_sec - t1.tv_sec)*1000 ;    // sec
  elapsedTime += (t2.tv_usec - t1.tv_usec) / 1000.0;   // us to ms
  elapsedTime/=1000.0;

   std::cout<<"Permutation time= "<<elapsedTime<<"\n";
   (*timeInfo)[8*1000+1]=elapsedTime;
  ...
  if (y_in == NULL){
    std::cout<<"----- Initialize Y \n";
    std::ifstream yin;
     yin.open("yin2m.txt");

    for(int i = 0; i < params.n; i++){
	for(int j=0;j<params.d;j++){
	//coord temp=randn() * .0001;
        coord temp;
	yin>>temp;
	y[iperm[i]*params.d+j] = temp;
//	yin<<temp<<" ";
	}
//	yin<<"\n";
    }
