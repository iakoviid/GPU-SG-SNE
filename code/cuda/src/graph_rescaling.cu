...
void lambdaRescalingGPU( sparse_matrix P, double lambda, bool dist, bool dropLeafEdge ){
  ...
  //int    maxIter     = 100;
  thrust::device_vector<coord> sig2(P.n);
  if (dist)  std::cout << "Input considered as distances" << std::endl;

  bisectionSearchKernel<<<64,Blocksize>>>(thrust::raw_pointer_cast(sig2.data()),P.val,P.col, P.row,P.n,lambda,tolBinary,dropLeafEdge);
