...
void cpuwarpsim(coord *V, coord *y, coord *q, int *ib, uint32_t *cb,
  ...
  int s = 0;
  int print=10;
  for (int boxIdx = 0; boxIdx < (ng - 3) * (ng - 3); boxIdx++) {
    if(ib[boxIdx]<0){continue;}
    //printf(" iboxIdx=%d ib[boxIdx]=%d ib[boxIdx+1]=%d\n", boxIdx, ib[boxIdx],
          // ib[boxIdx + 1]);
    // if(boxIdx>=(ng-2)*(ng-3)-1  ){return;}
    for (int TID = 0; TID < ib[boxIdx+1]-ib[boxIdx]; TID++) {
      //if(ib[boxIdx]==0 && boxIdx!=0){
      //  printf("boxIdx=%d\n",boxIdx );
      //  break;}

      // printf("ib[boxIdx]+ TID=%d  vs s=%d\n",ib[boxIdx]+ TID,s );
      if(TID+ib[boxIdx]!=s && print>0){
        print--;
        printf(" iboxIdx=%d ib[boxIdx]=%d ib[boxIdx+1]=%d\n", boxIdx, ib[boxIdx],
               ib[boxIdx + 1]);
                printf("ib[boxIdx]+ TID=%d  vs s=%d\n",ib[boxIdx]+ TID,s );
      ...
      f1 = (uint32_t)floor(y(0, ib[boxIdx] + TID));
      d = y(0, ib[boxIdx] + TID) - (coord)f1;
      ...
      f2 = (uint32_t)floor(y(1, ib[boxIdx] + TID));
      d = y(1, ib[boxIdx] + TID) - (coord)f2;
      ...
      // printf("i=%d f1 =%d ,f2=%d  \n",i,f1 ,f2  );
      // if(f1!=x &&
      // f2!=i){printf("____________________________________________________________________________maybe\n"
      // );}
      ...
      for (uint32_t j = 0; j < nVec; j++) {
        ...
        for (uint32_t idx2 = 0; idx2 < 4; idx2++) {
          ...
          coord qv2 = q(j, ib[boxIdx] + TID) * v2[idx2];

          for (uint32_t idx1 = 0; idx1 < 4; idx1++) {
            V[f1 + idx1+ (f2 + idx2)*ng +j*ng*ng ] += qv2 * v1[idx1];
          }
        }
      }
...
void cpuwarpsim3d(coord *V, coord *y, coord *q, int *ib, uint32_t *cb,
                  uint32_t ng, uint32_t nPts, uint32_t nDim, uint32_t nVec) {
  ...
  int print=10;
  uint32_t f1, f2, f3;
  for (int boxIdx = 0; boxIdx < (ng - 3) * (ng - 3) * (ng - 3); boxIdx++) {
    if(ib[boxIdx]<0){continue;}

    // if(boxIdx>=(ng-2)*(ng-3)*(ng-3)-1  ){return;}
    // printf("boxIdx=%d ib[boxIdx]=%d
    // ib[boxIdx+1]=%d\n",boxIdx,ib[boxIdx],ib[boxIdx+1] );

    for (int TID = 0; TID < ib[boxIdx + 1] - ib[boxIdx]; TID++) {
      coord d;
      // printf("boxIdx=%d ib[boxIdx]+ TID=%d  vs s=%d\n",boxIdx,ib[boxIdx]+
      // TID,s );
      if(TID+ib[boxIdx]!=s && print>0){
        print--;
        printf(" iboxIdx=%d ib[boxIdx]=%d ib[boxIdx+1]=%d\n", boxIdx, ib[boxIdx],
      ...
      f3 = (uint32_t)floor(y(2, ib[boxIdx] + TID));
      d = y(2, ib[boxIdx] + TID) - (coord)f3;
      ...
      for (uint32_t j = 0; j < nVec; j++) {
        ...
        for (uint32_t idx3 = 0; idx3 < 4; idx3++) {
          ...
          for (uint32_t idx2 = 0; idx2 < 4; idx2++) {
            ...
            coord qv2v3 = q(j, ib[boxIdx] + TID) * v2[idx2] * v3[idx3];
