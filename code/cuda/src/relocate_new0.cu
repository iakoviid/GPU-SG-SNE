...
__inline__  __device__ uint32_t untangleLastDimDevice(int nDim, int TID, uint32_t qLevel,
                                          uint64_t *C) {
  uint64_t mask;
  switch (nDim) {
  case 1:
...
__global__ void gridIdxKernelnew(int *ib,  uint64_t *C, int nPts,
                              int nDim, int nGrid, uint32_t qLevel) {
  for (register int TID = threadIdx.x + blockIdx.x * blockDim.x+1; TID < nPts;
       TID += gridDim.x * blockDim.x) {
         ...
         if(TID==nPts-1){ib[idxCur+1]=nPts; }
         if(TID==1){ib[gridIdxKernel(nDim, 0, qLevel, C,nGrid-1)]=0;}
