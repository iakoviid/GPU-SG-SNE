...
__global__ void
gpu_radix_sort_local(uint64_t *d_out_sorted, unsigned int *d_prefix_sums,
                     unsigned int *d_block_sums, unsigned int input_shift_width,
                     uint64_t *d_in, unsigned int d_in_len,
                     unsigned int max_elems_per_block, unsigned int split,
  ...
  //  scan mask ouput
  //  store needed value from current prefix sum array to merged prefix sum
  //  array store total sum of mask output (obtained from scan) to global block
  //  sum array
  // calculate local sorted address from local prefix sum and scanned mask
  // output's total sums shuffle input block according to calculated local
  // sorted addresses shuffle local prefix sums according to calculated local
  // sorted addresses copy locally sorted array back to global memory copy local
  // prefix sum array back to global memory
  ...
  unsigned int t_2bit_extract =
      (uint32_t)(t_data >> input_shift_width) & (split - 1);
  ...
  if (cpy_idx < d_in_len) {
    ...
    for (int dim = 0; dim < d; dim++) {
      Y_out[g_new_pos + dim * d_in_len] = Y_in[cpy_idx + dim * d_in_len];
    }
  }
}
template <class dataPoint>
__global__ void
gpu_glbl_shuffle(uint64_t *d_out, uint64_t *d_in,
                 unsigned int *d_scan_block_sums, unsigned int *d_prefix_sums,
                 unsigned int input_shift_width, unsigned int d_in_len,
                 unsigned int max_elems_per_block, unsigned int split,
                 uint32_t *iPerm_out, uint32_t *iPerm_in, dataPoint *Y_out,
                 dataPoint *Y_in, int d) {
  // get d = digit
  ...
  if (cpy_idx < d_in_len) {
    ...
    for (int dim = 0; dim < d; dim++) {
      Y_out[data_glbl_pos + dim * d_in_len] = Y_in[cpy_idx + dim * d_in_len];
    }
  }
}
__global__ void orderCheck(uint64_t *d, uint32_t n, uint32_t shift_width,
                           uint32_t split, int *order) {

  register int TID = threadIdx.x + blockIdx.x * blockDim.x;
  if (TID < n - 1) {
    uint32_t t_bit_extractcur = (uint32_t)(d[TID] >> shift_width) & (split - 1);
    uint32_t t_bit_extractnext =
        (uint32_t)(d[TID + 1] >> shift_width) & (split - 1);
...
__global__ void orderCheck3(uint64_t *d, uint32_t n, uint32_t shift_width,
                            uint32_t split, int *order) {
...
__global__ void orderCheck2(uint64_t *d, uint32_t n, uint32_t shift_width,
                            uint32_t split, int *order) {
  register int TID = threadIdx.x + blockIdx.x * blockDim.x;
...

{
  ...
  unsigned int shmem_sz = s_data_len * sizeof(uint64_t) +
                          +(s_mask_out_len + s_merged_scan_mask_out_len +
  ...
  for (unsigned int shift_width = 0; shift_width <= total;
       shift_width += bitStride) {

    orderCheck<<<grid_sz, block_sz>>>(d_in, d_in_len, shift_width, split,
                                      order);
    int outOfOrder = thrust::reduce(thrust::device, order, order + grid_sz, 0);
    // checkCudaErrors(cudaMemcpy(order_h, order, sizeof(bool) ,
    ...
    if (outOfOrder > 0) {
      ...
      gpu_radix_sort_local<<<grid_sz, block_sz, shmem_sz>>>(
          d_out, d_prefix_sums, d_block_sums, shift_width, d_in, d_in_len,
          max_elems_per_block, split, iPerm_out, iPerm_in, Y_out, Y_in, d);

      // unsigned int* h_test = new unsigned int[d_in_len];
      // checkCudaErrors(cudaMemcpy(h_test, d_in, sizeof(unsigned int) *
      // d_in_len, cudaMemcpyDeviceToHost)); for (unsigned int i = 0; i <
      // d_in_len; ++i)
      //    std::cout << h_test[i] << " ";
      // std::cout << std::endl;
      // delete[] h_test;
      ...
      gpu_glbl_shuffle<<<grid_sz, block_sz>>>(
          d_in, d_out, d_scan_block_sums, d_prefix_sums, shift_width, d_in_len,
  ...
  checkCudaErrors(cudaMemcpy(d_out, d_in, sizeof(uint64_t) * d_in_len,
                             cudaMemcpyDeviceToDevice));
  checkCudaErrors(cudaMemcpy(iPerm_out, iPerm_in, sizeof(uint32_t) * d_in_len,
                             cudaMemcpyDeviceToDevice));
  checkCudaErrors(cudaMemcpy(Y_out, Y_in, sizeof(dataPoint) * d * d_in_len,
                             cudaMemcpyDeviceToDevice));
...
template void radix_sort(uint64_t *const d_out, uint64_t *const d_in,
                         unsigned int d_in_len, uint32_t bitStride,
                         uint32_t total, uint32_t *iPerm_out,
                         uint32_t *iPerm_in, float *Y_out, float *Y_in, int d);
template void radix_sort(uint64_t *const d_out, uint64_t *const d_in,
                         unsigned int d_in_len, uint32_t bitStride,
                         uint32_t total, uint32_t *iPerm_out,
                         uint32_t *iPerm_in, double *Y_out, double *Y_in,
                         int d);
