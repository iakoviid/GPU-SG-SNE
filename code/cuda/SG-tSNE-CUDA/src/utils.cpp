...
coord randn() {
  coord x, y, radius;
  do {
    x = 2 * (rand() / ((coord) RAND_MAX + 1)) - 1;
    y = 2 * (rand() / ((coord) RAND_MAX + 1)) - 1;
    radius = (x * x) + (y * y);
...
void extractEmbedding( coord *y, int n, int d ){
  ...
  fwrite(y , sizeof(coord), n*d, pFile);
  fclose(pFile);
  return;
}
...
coord * readXfromMTX( const char *filename, int *n, int *d ){
  ...
  // allocate space for COO format
  X = static_cast<coord *>( malloc( n[0] * d[0] * sizeof(matval)) );
...
bool vdm_load_data(coord** data, int* n, int* d, int* no_dims, coord* theta, coord* perplexity, int* rand_seed, int* max_iter) {
  ...
  fread(theta, sizeof(coord), 1, h);      // gradient accuracy
  fread(perplexity, sizeof(coord), 1, h); // perplexity
  fread(no_dims, sizeof(int), 1, h);       // output dimensionality
  fread(max_iter, sizeof(int),1,h);        // maximum number of iterations
  *data = (coord*) malloc(*d * *n * sizeof(coord));
  if(*data == NULL) { printf("Memory allocation failed!\n"); exit(1); }
  fread(*data, sizeof(coord), *n * *d, h);         // the data
...
void vdm_save_data(coord* data, int* landmarks, coord* costs, int n, int d) {
  ...
  fwrite(data, sizeof(coord), n * d, h);
  fwrite(landmarks, sizeof(int), n, h);
  fwrite(costs, sizeof(coord), n, h);
  fclose(h);
