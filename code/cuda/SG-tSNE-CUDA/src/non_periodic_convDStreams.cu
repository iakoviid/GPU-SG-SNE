                     ...
                     ComplexD* Xc)
{
  ...
  setDataFft2D<<<grid, block, 0, streamRep>>>(Kc, Xc, n1, n2, nVec, VGrid, hsq, 1, 1);
  cufftExecZ2Z(plan, reinterpret_cast<cufftDoubleComplex *>(Kc),
               reinterpret_cast<cufftDoubleComplex *>(Kc), CUFFT_FORWARD);
  cufftExecZ2Z(plan_rhs, reinterpret_cast<cufftDoubleComplex *>(Xc),
               reinterpret_cast<cufftDoubleComplex *>(Xc), CUFFT_FORWARD);

  ComplexPointwiseMulAndScale<<<Blocks2D, Threads2D, 0, streamRep>>>(Xc, Kc, n1 * n2, nVec);

  cufftExecZ2Z(plan_rhs, reinterpret_cast<cufftDoubleComplex *>(Xc),
               reinterpret_cast<cufftDoubleComplex *>(Xc), CUFFT_INVERSE);
  addToPhiGrid<<<Blocks2D, Threads2D, 0, streamRep>>>(Xc, PhiGrid, n1 * n2 * nVec, (0.25 / (n1 * n2)));
...
void term3D(ComplexD *Kc, ComplexD *Xc, uint32_t n1, uint32_t n2, uint32_t n3,
  ...
  setDataFft3D<<<grid, block, 0, streamRep>>>(Kc, Xc, n1, n2, n3, nVec, VGrid, hsq, signx, signy,
                            signz);
  ...
  ComplexPointwiseMulAndScale<<<Blocks3D, Threads3D, 0, streamRep>>>(Xc, Kc, n1 * n2*n3, nVec);

  cufftExecZ2Z(plan_rhs, reinterpret_cast<cufftDoubleComplex *>(Xc),
               reinterpret_cast<cufftDoubleComplex *>(Xc), CUFFT_INVERSE);
  if(signx==-1 || signy==-1 || signz==-1){
  normalizeInverse3D<<<grid, block, 0, streamRep>>>(Xc, n1, n2, n3, nVec, signx, signy, signz);}
  addToPhiGrid<<<Blocks3D, Threads3D, 0, streamRep>>>(Xc, PhiGrid, n1 * n2 * n3 * nVec,
                            (0.125 / (n1 * n2 * n3)));
}
