...
static inline coord signc(coord x) {
...
void update_positionsCPU(dataPoint *const dY, dataPoint *const uY, int const N,
                         ...
                         dataPoint *const gains, coord const momentum,
                         coord const eta) {

  // Update gains
  for (int i = 0; i < N * no_dims; i++) {
    gains[i] = (signc(dY[i]) != signc(uY[i])) ? (gains[i] + .2) : (gains[i] * .8);
    if (gains[i] < .01)
      gains[i] = .01;
...
void kl_minimizationCPU(coord *y, tsneparams params, sparse_matrix P) {
  ...
  coord momentum = .5, final_momentum = .8;
  coord eta = 200.0;
  int iterPrint = 50;
