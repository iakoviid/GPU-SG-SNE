...
void hybrid_matrix_class<data_type>::allocate(csr_matrix_class<data_type> &matrix, double percent)
{
  ...
  auto avg_elements=statistic.avg_elements_in_rows;
  auto std_deviation= statistic.elements_in_rows_std_deviation;
   unsigned int elements_per_ell = avg_elements; //+ (max_elements - avg_elements) * percent;
  //thrust::device_vector<auto> d(row_ptr,row_ptr+n);
   cusp::array1d<int,cusp::host_memory> w(row_ptr,row_ptr+n+1);
   int optell= compute_optimal_entries_per_row(w);
   std::cout<<"Optimal:"<<optell<<" "<<"Avg:"<<elements_per_ell<<"\n";
    elements_per_ell=optell;
