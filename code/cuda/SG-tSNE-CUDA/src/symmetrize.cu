...
void add_cusparse(sparse_matrix A,sparse_matrix B,sparse_matrix *C){
  ...
  matidx *d_csrRowPtrC;
  cudaMalloc(&d_csrRowPtrC, (M + 1) * sizeof(uint32_t));
  ...
  coord *d_csrValC;    cudaMalloc(&d_csrValC, nnz3 * sizeof(coord));
  coord alpha = 1.f, beta = 1.f;
  cusparseDcsrgeam(handle, M, N, &alpha, descrA, A.nnz, A.val, A.row, A.col, &beta, descrB, B.nnz, B.val, B.row, B.col, descrC, d_csrValC, d_csrRowPtrC, d_csrColIndC);
...
void SymmetrizeMatrixGPU(sparsematrix P){

  unt32_t* csc_row_ptr_at = nullptr;
  cudaMalloc(reinterpret_cast<void**>(&csc_row_ptr_at),
          (P.nnz)*sizeof(int32_t));
  unt32_t* csc_column_ptr_at = nullptr;
  cudaMalloc(reinterpret_cast<void**>(&csc_column_ptr_at),
          (P.n+1)*sizeof(int32_t));
  coord* csc_values_at = nullptr;
  cudaMalloc(reinterpret_cast<void**>(&csc_values_at),
          (P.nnz)*sizeof(coord));
          cusparseHandle_t handle;

  // Do the transpose operation
  cusparseDcsr2csc(handle, P.m, P.n,
                  P.nnz, P.val, P.row,
  ...
  cusparseSetPointerMode(handle, CUSPARSE_POINTER_MODE_HOST);
  cusparseXcsrgeamNnz(handle, P.m, P.n,
          matrix_descriptor, P.nnz,P.row,
              P.col,
