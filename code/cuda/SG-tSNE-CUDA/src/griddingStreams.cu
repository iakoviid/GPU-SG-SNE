...
#define GRID_SIZE_THRESHOLD 600
...
initKernel<<<64, 1024, 0, streamRep>>>(VGridD,(double)0, szV);
  ...
  case 1:
    if (nGridDim <= GRID_SIZE_THRESHOLD) {
      ...
      s2g1drbwarp<<<Gridsz, BlockSizeWarp1D,0,streamRep>>>(VGrid, y, VScat, ib,
                                               nGridDim + 2, n, d, m);
    }
  ...
  case 3:
    if (nGridDim <= GRID_SIZE_THRESHOLD) {
      ...
      s2g3drbwarp<<<Gridsz, BlockSizeWarp3D,0,streamRep>>>(VGrid, y, VScat, ib,
                                               nGridDim + 2, n, d, m);
    }
    break;
  }
