...
int main() {
  ...
  HANDLE_CUSPARSE_ERROR(cusparseCreateMatDescr(&csrDescr),
                        "CSR Matrix descriptor initialization failed");
  ...
  HANDLE_CUSPARSE_ERROR(cusparseCreateMatDescr(&bsrDescr),
                        "BSR Matrix descriptor initialization failed");
  ...
  HANDLE_CUSPARSE_ERROR(
      cusparseXcsr2bsrNnz(handle, CUSPARSE_DIRECTION_ROW, m, n, csrDescr,
                          csrRowPtr, csrColInd, blockDim, bsrDescr, bsrRowPtr,
                          nnzTotalDevHostPtr),
  ...
  HANDLE_CUSPARSE_ERROR(
      cusparseScsr2bsr(handle, CUSPARSE_DIRECTION_ROW, m, n, csrDescr, csrVal,
                       csrRowPtr, csrColInd, blockDim, bsrDescr, bsrVal,
                       bsrRowPtr, bsrColInd),
  ...
  HANDLE_CUSPARSE_ERROR(
      cusparseScsrmm2(handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                      CUSPARSE_OPERATION_TRANSPOSE, m, dim, n, nnz, &fone,
                      csrDescr, csrVal, csrRowPtr, csrColInd, y, dim, &fzero,
                      z1, m),
      "cusparseScsrmm2 failed");

  HANDLE_ERROR(cudaMemcpy(z1HostPtr, z1, (size_t)(m * dim * sizeof(float)),
  ...
  HANDLE_CUSPARSE_ERROR(
      cusparseSbsrmm(
          ...
          CUSPARSE_OPERATION_TRANSPOSE, mb, dim, nb, nnzb, &fone, bsrDescr,
          bsrVal, bsrRowPtr, bsrColInd, blockDim, y, dim, &fzero, z2, m),
      "cusparseSbsrmm failed");

  HANDLE_ERROR(cudaMemcpy(z2HostPtr, z2, (size_t)(m * dim * sizeof(float)),
                          cudaMemcpyDeviceToHost));
  ...
  HANDLE_CUSPARSE_ERROR(cusparseDestroyMatDescr(csrDescr),
                        "CSR matrix descriptor destruction failed");
  csrDescr = 0;
  HANDLE_CUSPARSE_ERROR(cusparseDestroyMatDescr(bsrDescr),
                        "BSR matrix descriptor destruction failed");
  bsrDescr = 0;
