...
{  int opt;
  ...
  int nn = std::ceil( u*3 );
  params.lambda = 1; coord * X;
  int n, d;
  if(params.sim==0){
   X = readXfromMTX<coord>( filename.c_str(), &n, &d );
 }else{
   n=params.n;
  X =loadSimulationData<coord>(n,50);
  d=50;
}
   params.n = n;
   // ~~~~~~~~~~ run kNN search
   ...
   sparse_matrix<coord> P = perplexityEqualization<coord>( I, D, n, nn, u );
   ...
   free( D ); free( I );
     ...
     CUDA_CALL(cudaMemcpy(Pd.col, P.row,P.nnz * sizeof(int), cudaMemcpyHostToDevice));
     CUDA_CALL(cudaMemcpy(Pd.row, P.col,(P.n+1) * sizeof(int), cudaMemcpyHostToDevice));
     Pd.nnz=P.nnz;
     Pd.n=P.n;
     Pd.m=P.m;
   ...
   for(int i=0;i<n;i++){
     ...
     y =sgtsneCUDA<coord>( &Pd,  params,yin,NULL);
     free(yin);
}
else{
   y =sgtsneCUDA<coord>( &Pd,  params,NULL,NULL);
