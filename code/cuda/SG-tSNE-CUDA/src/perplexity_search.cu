...
sparse_matrix<dataPoint> perplexityEqualization(int *I, dataPoint *D, int num_points,
  ...
  dataPoint *val;
  matidx *row, *col;
  gpuErrchk(cudaMallocManaged(&val,num_near_neighbors * num_points * sizeof(dataPoint)));
  gpuErrchk(cudaMallocManaged(&col, num_near_neighbors * num_points * sizeof(matidx)));
  gpuErrchk(cudaMallocManaged(&row, (num_points+1) * sizeof(matidx)));
