...
void equalizeVertex(matval*  val_P,
		    matval * distances,
		    matval perplexity,
		    int nn){
  ...
  bool found = false;
  matval beta = 1.0;
  matval min_beta = -std::numeric_limits<matval>::max();
  matval max_beta =  std::numeric_limits<matval>::max();
  matval tol = 1e-5;

  // Iterate until we found a good perplexity
  int iter = 0; matval sum_P;
  while(!found && iter < 200) {
    ...
    sum_P = std::numeric_limits<matval>::min();
    ...
    if(Hdiff < tol && -Hdiff < tol) {
      ...
      if(Hdiff > 0) {
	min_beta = beta;
	if(max_beta == std::numeric_limits<matval>::max() || max_beta == -std::numeric_limits<matval>::max())
	  beta *= 2.0;
	else
	...
	max_beta = beta;
	if(min_beta == -std::numeric_limits<matval>::max() || min_beta == std::numeric_limits<matval>::max())
	  beta /= 2.0;
	else
...
sparse_matrix perplexityEqualization( int *I, matval *D, int n, int nn, matval u ){
