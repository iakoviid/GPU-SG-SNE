...
void AttractiveEstimation(sparse_matrix<dataPoint> P, int d, dataPoint *y,
  ...
  if (P.format == 0) {
    pq_csr_naive<dataPoint, int>
        <<<32, 256, 0, streamAttr>>>(P.row, P.col, P.val, P.n, d, y, Fattr);
    ...
    switch (d) {
    case 1:
      coo_pq_kernel<dataPoint, 1><<<64, 1024, 0, streamAttr>>>(
          P.nnz, P.col, P.row, P.val, y, Fattr, P.n);
      break;
    case 2:
      coo_pq_kernel<dataPoint, 2><<<64, 1024, 0, streamAttr>>>(
          P.nnz, P.col, P.row, P.val, y, Fattr, P.n);
      break;
    case 3:
      coo_pq_kernel<dataPoint, 3><<<64, 1024, 0, streamAttr>>>(
          P.nnz, P.col, P.row, P.val, y, Fattr, P.n);
      break;
    }
