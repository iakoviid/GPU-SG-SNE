  ...
  case 2:
    {
      ...
      printf("x=%d  y=%d\n",(uint32_t) ( ( C & mask ) >> qLevel ), (uint32_t) C);

      Cout = (uint32_t) ( ( C & mask ) >> qLevel )*ng+(uint32_t) C;
      break;
    }
  ...
  case 3:
    {
      ...
      Cout = (uint32_t) ( ( C & mask ) >> 2*qLevel )*ng*ng+(uint32_t) ( ( C & mask ) >> qLevel )*ng+(uint32_t) C ;
...
__inline__   uint32_t untangleLastDimDevice(int nDim,  uint32_t qLevel,
                                          uint64_t C) {
  uint64_t mask;
  switch (nDim) {
  case 1:
    mask = (1 << qLevel) - 1;
    return (uint32_t)(C & mask);

  case 2: {
    mask = (1 << 2 * qLevel) - 1;

    return (uint32_t)((C & mask) >> qLevel);
  }

  case 3: {
    mask = (1 << 3 * qLevel) - 1;

    return (uint32_t)((C & mask) >> 2 * qLevel);
  }
  }
  return 0;
}

__inline__  uint32_t gridIdxKernel(uint64_t C,int nDim,  uint32_t qLevel,
                                   int nGrid) {
switch (nDim) {
  case 1:
    return untangleLastDimDevice(1,  qLevel, C);
  case 2:
    return untangleLastDimDevice(2,  qLevel, C)*nGrid+untangleLastDimDevice(1,  qLevel, C);
  case 3:{
    //printf("1:%d   2:%d   3:%d\n",untangleLastDimDevice(3,  qLevel, C),untangleLastDimDevice(2,  qLevel, C),untangleLastDimDevice(1,  qLevel, C) );
    return untangleLastDimDevice(3,  qLevel, C)*nGrid*nGrid+untangleLastDimDevice(2,  qLevel, C)*nGrid+untangleLastDimDevice(1,  qLevel, C);
  }
}
}
...
void gridSizeAndIdx( uint32_t * const ib,
 ...
 uint32_t a=-1;
  for (uint32_t i = 0; i < nPts; i++){
    ...
    uint32_t idxNew = gridIdxKernel( C[i], nDim, qLevel,nGridDim-1 );
    //printf("idx[%d]=%d x=%d y=%d\n",i,idxNew,untangleLastDimDevice(2,  qLevel, C[i]) ,untangleLastDimDevice(2,  qLevel, C[i]) );
    cb[idxNew]++;
    if(idxNew!=a){
      //printf("idxNew=%d i=%d \n",idxNew,i );
      a=idxNew;
