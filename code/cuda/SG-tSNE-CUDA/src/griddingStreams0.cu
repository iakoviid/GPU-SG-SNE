...
void s2g(dataPoint *VGrid, dataPoint *y, dataPoint *VScat, uint32_t nGridDim,
  ...
  case 1:
    if (nGridDim <= GRID_SIZE_THRESHOLD) {
      ...
      s2g1drbwarp<<<Gridsz, BlockSizeWarp1D, 0, streamRep>>>(
  ...
  case 2:
    if (nGridDim <= GRID_SIZE_THRESHOLD) {
      ...
      s2g2drbwarpshared<<<Gridsz, BlockSizeWarp2Dshared, 0, streamRep>>>(
          VGrid, y, VScat, ib, nGridDim + 2, n, d, m);
    }
    break;
  ...
  case 3:
    if (nGridDim <= GRID_SIZE_THRESHOLD) {
      ...
      s2g3drbwarp<<<Gridsz, BlockSizeWarp3D, 0, streamRep>>>(
          VGrid, y, VScat, ib, nGridDim + 2, n, d, m);
    }
    break;
  }
...
void g2s(dataPoint *PhiScat, dataPoint *PhiGrid, dataPoint *y,
  ...
  case 1:
    g2s1d<<<64, 1024, 0, streamRep>>>(PhiScat, PhiGrid, y, nGridDim + 2, n, d,
                                      m);

    break;

  case 2:
    g2s2d<<<64, 1024, 0, streamRep>>>(PhiScat, PhiGrid, y, nGridDim + 2, n, d,
                                      m);
    break;

  case 3:
    g2s3d<<<64, 1024, 0, streamRep>>>(PhiScat, PhiGrid, y, nGridDim + 2, n, d,
                                      m);
    break;
  }
}
  ...
  for (register int TID = threadIdx.x + blockIdx.x * blockDim.x; TID < nPts;
    ...
    for (int j = 0; j < nVec; j++) {
      ...
      for (int idx2 = 0; idx2 < 4; idx2++) {
        dataPoint qv = q[j + nVec * TID] * v2[idx2];
...
template <class dataPoint>
__inline__ __device__ dataPoint warpReduce2V2D(dataPoint *partial_sum,
                                               dataPoint *V, uint32_t f1,
                                               uint32_t f2, uint32_t nVec,
...
__global__ void s2g2drbwarp(dataPoint *V, dataPoint *y, dataPoint *q, int *ib,
                            uint32_t ng, uint32_t nPts, uint32_t nDim,
                            uint32_t nVec) {
  __shared__ dataPoint partial_sum[4 * 4 * 3 * BlockSizeWarp2D];
    ...
    for (int TID = lane; TID < (int)ib[boxIdx + 1] - ib[boxIdx];
      ...
      for (uint32_t iVec = 0; iVec < nVec; iVec++) {
        ...
        for (idx2 = 0; idx2 < 4; idx2++) {
          dataPoint qv = q[iVec + (ib[boxIdx] + TID) * nVec] * v2[idx2];
          for (idx1 = 0; idx1 < 4; idx1++) {
            partial_sum[(iVec * 4 * 4 + idx2 * 4 + idx1) * BlockSizeWarp2D +
...
__global__ void s2g2drbwarpshared(dataPoint *V, dataPoint *y, dataPoint *q,
  ...
  uint32_t lane = tid % 32;
  uint32_t warpid = (uint32_t)tid / warpsize;
  register uint32_t boxIdx;
  // warp for every grid box
  for (uint32_t s = 0; s < 2; s++) { // red-black sync

    for (uint32_t idual = 6* warpid; idual < (ng - 3); idual += 6*Gridsz * BlockSizeWarp2D / warpsize) { // coarse-grid

      for (uint32_t ifine = 0; ifine < 3; ifine++) { // fine-grid

        // get index of current grid box
        uint32_t boxIdxLastDim = 3 * s + idual + ifine;
        // if above boundaries, break
        if (boxIdxLastDim > (ng - 4))
          break;
        for(int first_dim=0;first_dim<ng-3;first_dim++){
          boxIdx=boxIdxLastDim*(ng-3)+first_dim;
          if (ib[boxIdx] < 0) {
            continue;
          }
        for (uint32_t iVec = 0; iVec < nVec; iVec++) {
          ...
          for (int TID = lane; TID < (int)ib[boxIdx + 1] - ib[boxIdx];
            ...
            for (idx2 = 0; idx2 < 4; idx2++) {
              dataPoint qv = q[iVec + nVec * (ib[boxIdx] + TID)] * v2[idx2];
              for (idx1 = 0; idx1 < 4; idx1++) {
          ...
          warpReduce2V2Dshared(partial_sum, V, boxIdx % (ng - 3),
                               boxIdx / (ng - 3), iVec, ng, lane);
        }
      }}
    }
  }
