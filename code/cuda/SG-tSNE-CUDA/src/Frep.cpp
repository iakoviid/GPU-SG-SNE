...
coord computeFrepulsive_interpCPU(coord *Frep, coord *y, int n, int d, coord h,
                               int np,double* timeInfo) {
  ...
  start = tsne_start_timer();
  nuconvCPU(PhiScat, yt, VScat, ib, cb, n, d, d + 1, np, nGrid,timeInfo+1);
  if (timeInfo != NULL)
    timeInfo[5] = tsne_stop_timer("F&Z", start);
  ...
  */
  ...
  // start = tsne_start_timer();
  //printf("PhiScatCPU=%lf %lf %lf %lf %lf \n",PhiScat[0],PhiScat[1],PhiScat[2],PhiScat[3] );
  start = tsne_start_timer();
