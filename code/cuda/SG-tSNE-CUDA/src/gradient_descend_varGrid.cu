...
/*
  dataPoint miny[4];
    ...
    for (int j = 0; j < d; j++) {
    addScalar<<<Blocks, Threads, 0, streamRep>>>(thrust::raw_pointer_cast(Y.data())+j*n, -miny[j],n);
    }
*/
...
float compute_gradient(float *dy, double *timeFrep, double *timeFattr,
  ...
  for (int j = 0; j < d; j++) {
    ...
    addScalar<<<Blocks, Threads>>>(&y[j * n], -miny[j], n);
  }
  ...
  initKernel<<<Blocks, Threads>>>(Frep, (float)0.0, n * d);
  ...
  *timeFrep += timer.Elapsed();
  ...
  compute_dy<<<Blocks, Threads>>>(dy, Fattr, Frep, n, d, (float)params.alpha);
...
double compute_gradient(double *dy, double *timeFrep, double *timeFattr,
  ...
  initKernel<<<Blocks, Threads>>>(Frep, (double)0.0, n * d);
  ...
  *timeFrep += timer.Elapsed();
  ...
  cudaDeviceSynchronize();
  compute_dy<<<Blocks, Threads>>>(dy, Fattr, Frep, n, d, (double)params.alpha);
